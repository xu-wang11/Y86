...
instr_t instruction_set[] = 
{
    ...
    {"exchange",   HPACK(I_EXCHANGE, F_NONE), 6, M_ARG, 1, 0, R_ARG, 1, 1 },
...
//the initialization of sharing memory
void init_sharemem()
{
   
    shmid = shmget(SHMID, ADDR_END - ADDR_START, 0666|IPC_CREAT);
    sharedMem = shmat(shmid, (char *)0, 0);
}
void init_isa_share_memory()
{
    shmid = shmget(ISASHMID, ADDR_END - ADDR_START, 0666|IPC_CREAT);
    sharedMem = shmat(shmid, (char *)0, 0);
...
//the new version of function "copy_mem"
mem_t copy_mem(mem_t oldm)
{
	...
	{
		if(i >= ADDR_START && i < ADDR_END)
			newm->contents[i] = sharedMem[i-ADDR_START];
		else
...
bool_t diff_mem(mem_t oldm, mem_t newm, FILE *outfile)
{
	...
	ignore_share_get_word_val(oldm, pos, &ov);
...
//the original version of function "get_word_val"
...
//the new version of function "get_word_val"
bool_t get_word_val(mem_t m, word_t pos, word_t *dest)
{
	...
	if ((pos >= ADDR_START) && (pos + 4 < ADDR_END))
	{
		for (i = 0; i < 4; i++)
			val = val | sharedMem[pos - ADDR_START + i]<<(8*i);
		*dest = val;
	}
	else
	{
...
bool_t set_word_val(mem_t m, word_t pos, word_t val)
{
    ...
    if(pos >= ADDR_START && pos + 4 < ADDR_END)
	{
		for(i = 0; i < 4; i++)
		{
			sharedMem[pos - ADDR_START + i] = val & 0xFF;
			val >>= 8;
		}
	}
	else
...
bool_t diff_reg(mem_t oldr, mem_t newr, FILE *outfile)
{
    ...
    for (pos = 0; (!diff || outfile) && pos < len; pos += 4) {
        word_t ov = 0;
        word_t nv = 0;
	ignore_share_get_word_val(oldr, pos, &ov);
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVL || hi0 == I_ALU || hi0 == I_PUSHL ||
	 ...
	 hi0 == I_MRMOVL || hi0 == I_IADDL || hi0 == I_EXCHANGE);
	...
	(hi0 == I_IRMOVL || hi0 == I_RMMOVL || hi0 == I_MRMOVL ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_IADDL || hi0 == I_EXCHANGE);
...
{
    memset(sharedMem, 0, sizeof(char) * (ADDR_END - ADDR_START));
}
void remove_sharemem()
{
